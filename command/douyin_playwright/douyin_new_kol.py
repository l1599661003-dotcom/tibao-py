...
def get_pending_kols() -> List[Dict[str, Any]]:
    """获取需要处理的KOL列表"""
    try:
        api_url = "https://tianji.fangpian999.com/api/admin/creatorBusiness/getNewerCreator?platform_id=2"
        headers = {"Content-Type": "application/json"}

        response = requests.post(api_url, headers=headers, timeout=30, verify=False)
        creator_data = response.json()['data']
        # creator_data = [{'creator_nickname': '杨泡泡', 'platform_user_id': '6870162147838525453'}]
