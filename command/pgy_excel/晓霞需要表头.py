...
class PGYSpider:
    def __init__(self):
        ...
        self.required_columns = [
            '达人标签', '博主账号名', '蒲公英链接', '主页', '博主ID',
            '粉丝量（w）', '达人地域', '阅读中位数', '互动中位数', '图文裸价',
            '视频裸价', '图文结算价', '视频结算价', '推荐合作形式', '最终结算价',
            '小红书ID', '粉丝量（w）'
        ]
        
        # 笔记率数据存储
        self.notes_rate_data = {}  # 存储每个用户的笔记率数据
        ...
        try:
            ...
            for col in self.required_columns:
                if col in self.excel_data.columns:
                    # 将nan值替换为空字符串
                    self.excel_data[col] = self.excel_data[col].fillna('')
                    # 将字符串'nan'替换为空字符串
                    self.excel_data[col] = self.excel_data[col].replace('nan', '')
                    # 确保数据类型为字符串
                    self.excel_data[col] = self.excel_data[col].astype(str)
            ...
            for i in range(min(3, len(self.excel_data))):
                row_data = {}
                for col in self.required_columns[:5]:  # 只显示前5列
                    if col in self.excel_data.columns:
                        row_data[col] = self.excel_data.iloc[i][col]
        ...
        try:
            ...
            for index, row in self.excel_data.iterrows():
                try:
                    ...
                    user_id = self._extract_user_id_from_url(pgy_url)
                    try:
                    ...
                    if self.api_data:
                        ...
                        for api_url, response_data in api_data_copy.items():
                            ...
                            if 'blogger' in api_url:
                                self._update_excel_row(index, row, response_data)

                            elif 'notes_detail' in api_url:
                                self._process_notes_detail(index, row, response_data)
                            
                            # 处理第一次notes_rate API调用（页面加载时）
                            elif 'notes_rate' in api_url:
                                api_data = response_data.get('data', {})
                                if api_data and isinstance(api_data, dict) and api_data.get('code') == 0:
                                    # 存储第一次调用的数据
                                    self.notes_rate_data[user_id] = {
                                        'first_call': api_data.get('data', {}),
                                        'second_call': None
                                    }
                                    logger.info(f"存储第一次notes_rate数据，用户ID: {user_id}")
                    
                    # 点击"合作笔记"按钮获取笔记率数据
                    try:
                        self.api_data.clear()  # 清空之前的API数据
                            ...
                            if self.api_data:
                                ...
                                for api_url, response_data in notes_rate_copy.items():
                                    if 'notes_rate' in api_url and 'data' in response_data:
                                        api_data = response_data.get('data', {})
                                        if api_data and isinstance(api_data, dict) and api_data.get('code') == 0:
                                            logger.info("检测到第二次notes_rate API调用（点击合作笔记后）")
                                            
                                            # 如果已经有第一次调用的数据，比较两次数据
                                            if user_id in self.notes_rate_data:
                                                self.notes_rate_data[user_id]['second_call'] = api_data.get('data', {})
                                                
                                                # 比较两次数据，选择数值更高的
                                                final_data = self._compare_and_select_best_notes_rate(
                                                    self.notes_rate_data[user_id]['first_call'],
                                                    self.notes_rate_data[user_id]['second_call']
                                                )
                                                self.notes_rate_data[user_id] = final_data
                                                logger.info(f"已比较两次notes_rate数据，选择最优数据，用户ID: {user_id}")
                                            else:
                                                # 如果只有第二次调用，直接使用
                                                self.notes_rate_data[user_id] = api_data.get('data', {})
                                                logger.info(f"只有第二次notes_rate数据，用户ID: {user_id}")
                                            break
                        else:
                            logger.warning("未找到合作笔记按钮")
                            
                    except Exception as e:
                        logger.error(f"点击合作笔记时出错: {str(e)}")
                    
                    # 最终更新Excel数据（包含笔记率数据）
                    self._update_excel_row_final(index, row)
    ...
    def _compare_and_select_best_notes_rate(self, first_data, second_data):
        """比较两次notes_rate数据，选择数值更高的数据"""
        try:
            if not first_data and not second_data:
                return {}
            
            if not first_data:
                logger.info("第一次调用数据为空，使用第二次调用数据")
                return second_data
            
            if not second_data:
                logger.info("第二次调用数据为空，使用第一次调用数据")
                return first_data
            ...
            for field in compare_fields:
                first_value = self._safe_get_numeric(first_data, field, 0)
                second_value = self._safe_get_numeric(second_data, field, 0)
            ...
            if second_score > first_score:
                logger.info("第二次调用数据更优，选择第二次数据")
                return second_data
            else:
                logger.info("第一次调用数据更优，选择第一次数据")
                return first_data
                
        except Exception as e:
            logger.error(f"比较notes_rate数据时出错: {str(e)}")
            # 出错时返回第一次数据作为默认值
    ...
    def _update_excel_row(self, index, row, api_data):
        """更新Excel行数据"""
        try:
            ...
            # 从蒲公英链接中提取ID作为小红书ID
            pgy_url = row.get('蒲公英链接', '')
            part = self._extract_user_id_from_url(pgy_url)
            self.excel_data.at[index, '博主ID'] = str(part)
            self.excel_data.at[index, '主页'] = f"https://www.xiaohongshu.com/user/profile/{part}"

            logger.info(f"响应数据结构: {api_data}")

            data = api_data['data']['data']
            logger.info(f"API数据: {data}")
            ...
            self.excel_data.at[index, '博主账号名'] = nickname

            # 更新博主ID
            red_id = safe_get(data, 'redId', '')
            self.excel_data.at[index, '小红书ID'] = red_id

            # 更新粉丝量（w）
            fans_count = safe_get(data, 'fansCount', 0)
            self.excel_data.at[index, '粉丝量（w）'] = fans_count
            ...
            if content_tags and isinstance(content_tags, list):
                for content_tag in content_tags:
                    ...
                    if safe_get(content_tag, 'taxonomy1Tag'):
                        tags_list.append(content_tag['taxonomy1Tag'])
                    taxonomy2_tags = safe_get(content_tag, 'taxonomy2Tags', [])
                    if taxonomy2_tags and isinstance(taxonomy2_tags, list):
                        tags_list.extend([tag for tag in taxonomy2_tags])

            # 处理featureTags
            feature_tags = safe_get(data, 'featureTags', [])
            if feature_tags and isinstance(feature_tags, list):
                tags_list.extend([tag for tag in feature_tags])

            # 组合标签
            combined_tags = ', '.join(tags_list) if tags_list else ''
            if combined_tags:
                self.excel_data.at[index, '达人标签'] = combined_tags

            # 更新图文裸价
            picture_price = safe_get(data, 'picturePrice', 0)
            if picture_price:
                self.excel_data.at[index, '图文裸价'] = picture_price

            # 更新视频裸价
            video_price = safe_get(data, 'videoPrice', 0)
            if video_price:
                self.excel_data.at[index, '视频裸价'] = video_price

            # 更新达人地域
            location = safe_get(data, 'location', '')
            if location:
                self.excel_data.at[index, '达人地域'] = location

        except Exception as e:
            logger.error(f"更新Excel行数据时出错: {str(e)}")
            logger.error(f"错误详情: {traceback.format_exc()}")

    def _process_notes_detail(self, index, row, api_data):
        """更新Excel行数据"""
        try:

            datas = api_data['data']['data']
            logger.info(f"笔记API数据: {datas}")
            ...
            self.excel_data.at[index, '推荐合作形式'] = recommended_form

        except Exception as e:
            logger.error(f"更新Excel行数据时出错: {str(e)}")
            logger.error(f"错误详情: {traceback.format_exc()}")

    def _update_excel_row_final(self, index, row):
        """最终更新Excel行数据，包含笔记率数据和推荐合作形式"""
        try:
            # 数据类型转换函数
            ...
            # 获取笔记率数据（经过比较后的最优数据）
            notes_rate_data = self.notes_rate_data.get(user_id, {}) if user_id else {}
            
            # 更新阅读中位数和互动中位数
            if notes_rate_data:
                logger.info(f"处理用户 {user_id} 的笔记率数据: {notes_rate_data}")
                
                # 阅读中位数
                read_median = notes_rate_data.get('readMedian', 0)
                if read_median:
                    self.excel_data.at[index, '阅读中位数'] = safe_convert_to_str(read_median)
                    logger.info(f"设置阅读中位数: {read_median}")
                
                # 互动中位数 - 尝试多个可能的字段名
                engage_median = (notes_rate_data.get('mEngagementNum', 0) or 
                               notes_rate_data.get('interactionMedian', 0))
                if engage_median:
                    self.excel_data.at[index, '互动中位数'] = safe_convert_to_str(engage_median)
                    logger.info(f"设置互动中位数: {engage_median}")
            else:
                # 如果没有笔记率数据，设置默认值
                self.excel_data.at[index, '阅读中位数'] = ''
                self.excel_data.at[index, '互动中位数'] = ''
                logger.warning(f"用户 {user_id} 没有笔记率数据")
                
        except Exception as e:
            logger.error(f"最终更新Excel行数据时出错: {str(e)}")
            logger.error(f"错误详情: {traceback.format_exc()}")
    ...
    def _handle_api_response(self, response):
        ...
        try:
            url = response.url
            # 设置更广泛的目标API关键词
            target_apis = ['user/blogger', 'notes_rate', 'notes_detail']
