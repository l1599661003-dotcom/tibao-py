...
class PGYSpider:
    def __init__(self):
        ...
        self.required_columns = [
            ...
            '所属机构', '前一页视频笔记占比',
            '合作曝光中位数', '合作阅读中位数', '合作互动中位数', '外溢进店中位数'
        ]
    ...
    def _handle_api_response(self, response):
        ...
        try:
            ...
            target_apis = ['user/blogger', 'notes_detail', 'data_summary']
    ...
    def _process_data_summary(self, api_data, row_index):
        ...
        try:
            ...
            # 安全获取各个字段值
            try:
                self.excel_data.at[row_index, '合作曝光中位数'] = str(safe_numeric_value(data.get('mAccumImpNum')))
                self.excel_data.at[row_index, '合作阅读中位数'] = str(safe_numeric_value(data.get('readMedian')))
                self.excel_data.at[row_index, '合作互动中位数'] = str(safe_numeric_value(data.get('mEngagementNum')))
                self.excel_data.at[row_index, '外溢进店中位数'] = str(safe_numeric_value(data.get('mCpuvNum')))
            except Exception as e:
    ...
    def _process_notes_detail(self, api_data, row_index):
        ...
        try:
            ...
            # 更新Excel数据
            try:
                self.excel_data.at[row_index, '前一页视频笔记占比'] = str(video_note_ratio)
            except Exception as e:
