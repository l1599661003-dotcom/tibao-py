...
def fetch_blogger_intro(blogger_id: str, session: requests.Session) -> Optional[str]:
    ...
    url = f"https://www.douyin.com/user/MS4wLjABAAAA_DDypyKYHfO1sJ8ZejhRVHeYdrBiVfGHFwSKCUqaplT-pkcMeIaVWxUZHpx-Fz61?from_tab_name=main"
    print(f"访问博主主页: {url}")

    try:
        time.sleep(Config.REQUEST_DELAY)
        response = session.get(url, headers=DY_HEADERS, timeout=Config.REQUEST_TIMEOUT)
        print(response.text)
        exit()
        response.raise_for_status()
