...
class PGYSpider:
    ...
    def scrape_user_notes(self):
        ...
        try:
            ...
            for url in urls:
                if not url['platform_user_id'] or "aaaaaaaaa" in url['platform_user_id']:
                    continue
                ...
                try:
                    ...
                    # 访问页面
                    page_url = f"https://pgy.xiaohongshu.com/solar/pre-trade/blogger-detail/{url['platform_user_id']}"
                    ...
                    if not self.api_data:
                        logger.info(f"未找到API数据，跳过处理博主 {url['creator_nickname']}")
                        continue
                    ...
                    if sync_result:
                        logger.info(f"成功同步博主 {url['creator_nickname']} 的数据到API")
                    else:
                        logger.warning(f"同步博主 {url['creator_nickname']} 的数据到API失败")

                except Exception as e:
                    logger.error(f"处理博主 {url['creator_nickname']} 数据时出错: {str(e)}")
                    continue
