...
class PGYSpider:
    ...
    def scrape_user_notes(self):
        ...
        try:
            ...
            try:
                urls = new_session.query(TrainingBloggerDetails).filter(
                    TrainingBloggerDetails.month == self.peizhi_month,
                    TrainingBloggerDetails.status == 0,
                    (TrainingBloggerDetails.video_price >= self.peizhi_video_price) | (TrainingBloggerDetails.graphic_price >= self.peizhi_graphic_price),
                    # TrainingBloggerDetails.video_price >= self.peizhi_video_price,
                    # TrainingBloggerDetails.graphic_price >= self.peizhi_graphic_price,
                    TrainingBloggerDetails.id >= self.peizhi_start_id,
                    TrainingBloggerDetails.id <= self.peizhi_end_id,
                    TrainingBloggerDetails.blogger_dandelion_id.isnot(None),
                    TrainingBloggerDetails.blogger_dandelion_id != ''
                ).all()
        ...
        if 'list' in data and data['list']:
            ...
            for item in data['list']:
                try:
                    ...
                    logger.warning(f"解析笔记数据失败: {item}, 错误: {str(e)}")
                    continue
                ...
                # 如果没有上个月的数据，返回False表示停止分页
                if not has_last_month_data:
                    logger.info("没有上个月的数据，停止分页")
                    return False

                logger.info("包含上个月的数据，继续分页")
                return True
