...
def main():
    """
    主函数 - 外采博主数据抓取程序（无限循环版本）
    """
    logger.info("=== 外采博主数据抓取程序启动（无限循环模式）===")
    logger.info("程序将持续运行，每轮处理完成后等待5分钟再继续...")
    ...
    while True:
        try:
            ...
            if success:
                logger.info("✅ 本轮任务执行成功")
            else:
                logger.warning("⚠️ 本轮任务执行失败，将在5分钟后重试")

            logger.info("等待5分钟后继续下一轮查询...")
