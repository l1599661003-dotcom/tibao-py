...
def get_driver_path():
    if getattr(sys, 'frozen', False):
        # exe模式，取exe同级目录
        return os.path.join(os.path.dirname(sys.executable), 'ms-playwright')
    else:
        # 脚本模式，取py文件同级目录
        return os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ms-playwright')

os.environ['PLAYWRIGHT_BROWSERS_PATH'] = get_driver_path()
...
class QianguaSpider:
    def __init__(self):
        ...
        self.api_data = {}
        self.cookie_file = os.path.join(get_app_dir(), 'cookies.json')
        self.setup_browser()

    """设置日志"""
    def setup_logger(self):
        log_path = os.path.join(get_app_dir(), 'logs')
                ...
                if response.status == 200:
                    try:
                        ...
                        if 'Data' in response_data and 'ItemList' in response_data['Data']:
                            ...
                            logger.info(f"收到API响应: timestamp={timestamp}, URL={url[:100]}..., 数据条数={items_count}")
    ...
    def scrape_rank_data(self):
        ...
        try:
            ...
            categories = ["彩妆", "护肤", "洗护香氛", "母婴育儿", "家居家装", "科技数码", "情感两性", "星座情感", "生活经验"]
